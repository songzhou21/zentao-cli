...
fn spawn_test_server(plans: Vec<ResponsePlan>) -> (String, Arc<Mutex<Vec<String>>>, thread::JoinHandle<()>) {
...
// VerifyCookie 应覆盖成功、登录跳转、非预期跳转和 HTTP 非 2xx。
#[test]
...
fn fetch_bug_html_table_cases() {
    ...
    for (_name, plans, want_err, want_body_contains) in cases {
        ...
        assert!(
            body.contains(want_body_contains),
