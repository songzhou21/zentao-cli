...
- Bug 资源入口：`zentao bug list`、`zentao bug stats`、`zentao bug view <ID|URL>`、`zentao bug selection --build`。
- `bug selection` 只出 JSON（`value`,`name`）。`--build` 列出版本候选；`value` 用于 `--opened-build` / `--resolved-build`。可跟关键词做名称包含过滤。无人类表格。
...
- `list`/`stats` 版本筛选：`--opened-build`（影响版本）、`--resolved-build`（解决版本）；数字当作版本 ID 原样提交；非数字按候选名称唯一包含匹配后转 ID 再搜。多个/零个匹配时报错，并指向 `zentao bug selection --build`。对应禅道 `openedBuild` / `resolvedBuild`，操作符 `=`。
